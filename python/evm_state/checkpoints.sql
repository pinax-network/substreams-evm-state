...
) ENGINE = ReplacingMergeTree PARTITION BY snapshot_id ORDER BY (snapshot_id, address, slot)
...
) ENGINE = ReplacingMergeTree PARTITION BY snapshot_id ORDER BY (snapshot_id, address)
SETTINGS fsync_after_insert=1, fsync_part_directory=1;

CREATE TABLE IF NOT EXISTS checkpoints (
    snapshot_id String, block_number UInt64, block_hash String, created_at UInt64,
    manifest String
) ENGINE = ReplacingMergeTree PARTITION BY snapshot_id ORDER BY snapshot_id
SETTINGS fsync_after_insert=1, fsync_part_directory=1;

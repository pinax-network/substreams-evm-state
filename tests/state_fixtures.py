...
from evm_state.cursor import PUBLIC_KEY, PUBLIC_NONCE, save_progress
...
def insert_blocks(client, blocks):
    ...
    client.insert("_blocks_", [{"number": row["number"], "hash": row["hash"][2:], "timestamp": row["timestamp"],
                               "version": i + 1, "deleted": False} for i, row in enumerate(blocks)])


def native_cursor(number, block_hash):
    # Synthetic transport metadata; decoder compatibility is also tested against
    # the committed vectors generated by the upstream Go implementation.
    ref = f"{number}:{block_hash.removeprefix('0x')}"
    payload = f"c1:17:{ref}:{ref}".encode()
    return base64.urlsafe_b64encode(SecretBox(PUBLIC_KEY).encrypt(payload, PUBLIC_NONCE).ciphertext).decode()


def source(client, start, accounts=(A,), target=None):
    ...
    if not (directory / "run.json").exists():
        ...
        identity = {"format_version": 3, "database": client.database, "http_url": client.url,
            "checkpoint_database": target.database if target else client.database,
    ...
    tip = list(client.rows("SELECT number,hash FROM state_blocks FINAL ORDER BY number DESC,hash LIMIT 1"))
    if tip:
        record = json.loads((directory / "run.json").read_text())
        token = native_cursor(tip[0]["number"], tip[0]["hash"])
        save_progress(client, record, directory, token)
        (directory / "cursor.txt").write_text(token)
